...
user_movie_df = create_user_movie_df() #filtrelenen pivot tablomuz
...
movies_watched_df = user_movie_df[movies_watched] #genel df içerisindeki random kullanıcını izlediği filmleri al
...
# kullanıcıların random ile ortak izledikleri filmlerin toplamı
user_movie_count = movies_watched_df.T.notnull().sum() # notnull() => true false döner => bu dönüşümün trueları sum ediliyor (1)

# movies_watched_df.T => dataframein satırları(userId) sütuna geçecek
...
latest_watched = user_info.loc[user_info["timestamp"] == user_info["timestamp"].max()]
latest_highest_rating = user_info.loc[user_info["rating"] == user_info["rating"].max()]

latest_highest_rated_date = latest_highest_rating.loc[latest_highest_rating["timestamp"].idxmax()]
